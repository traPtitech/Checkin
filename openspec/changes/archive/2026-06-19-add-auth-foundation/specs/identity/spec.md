...
システムは、isct メールの平文を DB に保存しては SHALL NOT。本人識別に必要なのは `mail_hash` のみであり、メール平文は確認メールの送信など処理中の一時的な利用に限る。メール平文の保持が必要な場面（請求先など）は Stripe 側に委ねる。

#### Scenario: 確認後に平文が残らない

- **WHEN** isct メール確認が完了し本人行が作成される
- **THEN** `users` 行にはメール平文を保持する列が存在せず、`mail_hash` のみが保存される
