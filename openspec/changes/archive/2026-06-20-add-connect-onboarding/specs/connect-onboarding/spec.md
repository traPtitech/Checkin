...
システムは、受取人（`users` 行）に対し Stripe Connect の connected account を解決 SHALL：`users.stripe_connected_account_id` があれば再利用し、無ければ作成して保存する MUST。作成する account は Express とし、`metadata` に `mail_hash` を入れてよいが、対応付けの参照キーは `stripe_connected_account_id` とする。
...
onboarding リンク発行と状態確認は会計（管理者）のみがアクセス可能 SHALL。認可チェックは入力検証より先に実行される MUST。
