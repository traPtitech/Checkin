...
会計（管理）向けの Web UI（`@nuxt/ui`）。`/payments`（入出金一覧: 請求書/決済セッションのタブ・status フィルタ・カーソルページネーション・Stripe Dashboard リンク）と `/payouts`（払い戻し管理: Jomon 取込実行・status フィルタ一覧・行ごとの execute／onboarding リンク発行・onboarding 状態確認）。サーバ状態は既存 oRPC（`payments.listInvoices`/`listCheckoutSessions`、`payouts.list`/`processApproved`/`execute`/`createOnboardingLink`/`onboardingStatus`、`auth.me`）越しに扱う。すべて `adminProc`（会計セッション必須）で、UI の会計ガードは導線・体験のため（認可の source of truth はサーバ）。
