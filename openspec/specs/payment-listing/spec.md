...
会計向けの入出金一覧。Stripe を source として、請求書由来（Invoices）・決済ページ由来（Checkout Sessions）の 2 系統を、`status` フィルタとカーソルページネーションで読み取り専用に提供する。行は Stripe 型に依存しないドメイン DTO に正規化し、Dashboard URL を添える。

## Requirements

### Requirement: 会計のみがアクセスできる

入出金一覧は会計（管理者）のみがアクセス可能 SHALL。利用者・未ログインのアクセスは拒否する MUST。認可チェックは入力検証より先に実行され、未認可の呼び出しに入力スキーマの形を漏らしては SHALL NOT。
...
#### Scenario: 未認可なら入力検証より前に拒否

- **WHEN** 未ログインの呼び出しが不正な入力で一覧を要求する
- **THEN** 入力検証エラー（形の漏洩）ではなく認可エラーが返る
...
システムは、各行を Stripe 型に依存しないドメイン DTO に正規化して返す SHALL。各行は少なくとも次を含む MUST：`id`、金額（最小単位＋通貨）、日時、customer 参照（id、可能なら名前）、支払い状況、支払いの id、商品（Price）参照、および Stripe Dashboard で開ける URL。値が欠落する場合は明示的な null を返し、欠落で例外を投げては SHALL NOT。

#### Scenario: 行に必要項目が含まれる

- **WHEN** 一覧の各行を受け取る
- **THEN** `id`・金額・通貨・日時・customer 参照・支払い状況・支払い id・商品参照（無ければ null）・Dashboard URL が含まれる

#### Scenario: 欠落フィールドは null で安全に返す

- **WHEN** customer 削除済みや未展開などでフィールドが欠落する行を正規化する
- **THEN** 例外を投げず、該当フィールドは null として返る
