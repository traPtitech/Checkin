...
describe('toListResponse', () => {
  ...
  it('has_more が true なら next_cursor は data の末尾要素の id', () => {
    const page = { has_more: true, data: [{ id: 'a' }, { id: 'b' }, { id: 'c' }] }
    // 末尾(c)を選ぶ。先頭(a)を返す実装への退行を捕まえる。
    expect(toListResponse(page, id).next_cursor).toBe('c')
  })

  it('has_more が false なら next_cursor は null', () => {
    const page = { has_more: false, data: [{ id: 'a' }, { id: 'b' }] }
    expect(toListResponse(page, id).next_cursor).toBeNull()
  })

  it('has_more が true でも data が空なら next_cursor は null', () => {
    // Stripe は has_more:true で空 data を返さないが、undefined を漏らさず null に倒す。
    const data: { id: string }[] = []
    expect(toListResponse({ has_more: true, data }, id).next_cursor).toBeNull()
  })
  ...
  it('data は toView で射影し、カーソルは Stripe ソースの id から導出する', () => {
    ...
    expect(result.next_cursor).toBe('src_2')
  })
})
