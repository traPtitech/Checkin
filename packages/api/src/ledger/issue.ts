...
export async function resolveIssuance(db: Database, target: LedgerTarget): Promise<IssuanceResolution> {
  ...
  if (existing.length > 0) {
    ...
    const soleChargeGroup = chargeGroups.length === 1 ? chargeGroups[0] : undefined
    if (soleChargeGroup !== undefined) {
      const groupSlots = await getSlotsByChargeGroup(db, soleChargeGroup)
