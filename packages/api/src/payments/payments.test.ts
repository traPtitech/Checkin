...
/**
 * Representative plain objects shaped like the Stripe responses. The normalizers
 * only ever read the handful of fields exercised here, so the fixtures spell out
 * those and stand in for the full Stripe type. `fixture` is the single place in
 * this file where that shortcut is taken; it is used only to build fixtures.
 */
function fixture(value: unknown): never {
  // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- deliberate test fixture: a partial object stands in for the full Stripe type
...
describe('invoiceToRow', () => {
  ...
  it('tolerates a missing line / payments / status', () => {
    const row = invoiceToRow(
      makeInvoice({ status: null, lines: fixture({ data: [] }), payments: undefined }),
      'test',
    )
  ...
  it('drops the name for a deleted (expanded) customer', () => {
    const row = invoiceToRow(
      makeInvoice({ customer: fixture({ id: 'cus_del', deleted: true, object: 'customer' }) }),
      'test',
    )
  ...
  it('handles a null amount / currency / customer and an expanded intent', () => {
    const row = checkoutSessionToRow(
      makeSession({
        ...
        payment_intent: fixture({ id: 'pi_exp' }),
      }),
      'live',
    )
