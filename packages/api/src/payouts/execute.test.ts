...
function fakeStripe(opts: { transferOk?: boolean } = {}): StripeClient {
  const sdk = {
    accounts: {
      create: () => Promise.resolve({ id: `acct_${randomUUID().slice(0, 8)}` }),
      del: () => Promise.resolve({}),
    },
    accountLinks: {
      create: () => Promise.resolve({ url: 'https://connect.stripe.test/onboard' }),
    },
    transfers: {
      create: () => {
        if (opts.transferOk === false) {
          return Promise.reject(new Error('stripe transfer declined'))
        }
        return Promise.resolve({ id: `tr_${randomUUID().slice(0, 8)}` })
      },
    },
  }
  // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- deliberate test stub: only the SDK surface the orchestration calls is supplied
...
describe('processApprovedPayouts (orchestration)', () => {
  it('auto-creates a payout-only user for an unlinked traQ ID and parks it as onboarding_waiting', async () => {
    ...
    if (!minted) {
      throw new Error(`no user was minted for traqId ${traqId}`)
    }
  ...
  it('pays out a done payee and writes the result back to Jomon', async () => {
    ...
    expect(jomon.writeBacks).toHaveLength(1)
    expect(jomon.writeBacks[0]?.jomonRef).toBe(ref)
    expect(jomon.writeBacks[0]?.result).toMatchObject({ status: 'paid' })
  })
  ...
  it('marks failed and writes back when the transfer is declined', async () => {
    ...
    expect(jomon.writeBacks).toHaveLength(1)
    expect(jomon.writeBacks[0]?.jomonRef).toBe(ref)
    expect(jomon.writeBacks[0]?.result).toMatchObject({ status: 'failed' })
  })
  ...
  it('keeps the payout paid when v2 write-back is unsupported, leaving it for retry', async () => {
    ...
    class UnsupportedWriteBackJomon extends StubJomonClient {
      override writeBackResult(): Promise<void> {
        return Promise.reject(new JomonWriteBackUnsupportedError('v2 write-back unsupported'))
      }
    }
  ...
  it('returns a summary with listError (not a throw) when the approved-requests fetch fails', async () => {
    ...
    class ListFailsJomon extends StubJomonClient {
      override listApprovedTransferRequests(): Promise<JomonTransferRequest[]> {
        return Promise.reject(new Error('jomon list boom'))
      }
    }
...
describe('markPayoutManuallyPaid (manual bank transfer settle)', () => {
  ...
  it('isolates a write-back failure: the payout stays paid and is left for retry', async () => {
    ...
    class FlakyJomon extends StubJomonClient {
      override writeBackResult(): Promise<void> {
        return Promise.reject(new Error('jomon write-back boom'))
      }
    }
