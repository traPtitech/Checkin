...
import type { JomonTransferRequest } from '../jomon/types'
...
/** Unique suffix so repeated runs don't collide on jomon_ref / mail_hash. */
...
/** Insert a payee user with a known mail_hash; returns its email + id. */
async function makePayee(localPart: string, onboarding: 'none' | 'done'): Promise<{ email: string, id: string }> {
  const email = `${localPart}-${tag}@m.isct.ac.jp`
  const id = randomUUID()
...
function req(jomonRef: string, payeeEmail: string, amount = 4000): JomonTransferRequest {
  refs.push(jomonRef)
  return { jomonRef, payeeEmail, amount, currency: 'jpy' }
}
...
describe('processApprovedPayouts (orchestration)', () => {
  it('leaves an unidentifiable payee as pending and does not pay out', async () => {
    if (!available) {
      return
    }
    const jomon = new StubJomonClient([req(`jmn-unres-${tag}`, `nobody-${tag}@m.isct.ac.jp`)])
  ...
  it('parks a resolved-but-not-onboarded payee as onboarding_waiting', async () => {
    if (!available) {
      return
    }
    const payee = await makePayee('waiting', 'none')
    const jomon = new StubJomonClient([req(`jmn-wait-${tag}`, payee.email)])
    const summary = await processApprovedPayouts(
      { db, stripe: fakeStripe(), jomon },
      config,
    )
  ...
  it('pays out a done payee and writes the result back to Jomon', async () => {
    ...
    const jomon = new StubJomonClient([req(ref, payee.email)])
    const summary = await processApprovedPayouts(
  ...
  it('flags a userId mismatch for manual review and does not relink/transfer', async () => {
    ...
    const jomon = new StubJomonClient([req(ref, other.email)])
    const summary = await processApprovedPayouts(
      { db, stripe: fakeStripe({ transferOk: true }), jomon },
      config,
    )
  ...
  it('isolates a per-item error so the rest of the batch still runs', async () => {
    if (!available) {
      return
    }
    ...
    // A stub whose write-back throws ONLY for the bad ref, so its item errors.
    class FlakyJomon extends StubJomonClient {
      override async writeBackResult(jomonRef: string, result: { status: 'paid' | 'failed', stripeTransferId?: string, message?: string }): Promise<void> {
        if (jomonRef === badRef) {
    ...
    const jomon = new FlakyJomon([req(badRef, badPayee.email), req(goodRef, goodPayee.email)])

    const summary = await processApprovedPayouts(
      { db, stripe: fakeStripe({ transferOk: true }), jomon },
      config,
    )
    // The good item still paid out despite the bad item throwing.
    expect(summary.paid).toBe(1)
    expect(summary.errored).toBe(1)
    expect(summary.errors).toContain(badRef)
