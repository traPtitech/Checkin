...
import { affectedRowCount } from '../mysql-result'
...
export async function claimPayoutForExecution(
  ...
  // 1 ⇒ we won the claim, 0 ⇒ already paid/processing.
  const claimed = affectedRowCount(result)
  return claimed === 1
}
...
export async function settlePayoutManually(
  ...
  // As in `claimPayoutForExecution`: 1 ⇒ we settled it, 0 ⇒ it was already
  // paid/processing (the claimable WHERE matched nothing).
  const claimed = affectedRowCount(result)
  return claimed === 1
}
