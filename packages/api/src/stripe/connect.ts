...
export function constructAccountEvent(
  ...
  // `Stripe.Event` is discriminated by `type`, and only `account.updated` carries
  // a connected `Account`. Any other event type keeps the payouts fields at the
  // values the previous unchecked read produced for a non-Account payload.
  const account = event.type === 'account.updated' ? event.data.object : null
  return {
    id: event.id,
    type: event.type,
    // `event.account` is the connected account for Connect events; fall back to
    // the object id for direct-account deliveries.
    accountId: event.account ?? ('id' in object && typeof object.id === 'string' ? object.id : null),
    account: {
