...
function fakeStripe() {
  const create = vi.fn((_params: InvoiceCreateParams) => Promise.resolve({ id: 'in_test_123' }))
  const itemCreate = vi.fn((_params: InvoiceItemParams) => Promise.resolve({ id: 'ii_test_123' }))
  const del = vi.fn((_id: string) => Promise.resolve({}))
  const sdk = {
    invoices: { create, del },
    invoiceItems: { create: itemCreate },
  }
  // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- deliberate test stub: only the SDK surface `createDraftInvoice` calls is supplied
...
describe('createDraftInvoice', () => {
  it('offers card and bank transfer (jp_bank_transfer via customer_balance)', async () => {
    ...
    if (!arg) {
      throw new Error('invoices.create was not called')
    }
  ...
  it('attaches the chosen price as the single line item', async () => {
    ...
    if (!arg) {
      throw new Error('invoiceItems.create was not called')
    }
    expect(arg.customer).toBe('cus_abc')
