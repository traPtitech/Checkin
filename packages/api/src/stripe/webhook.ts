...
export function constructEvent(
  ...
  // `event.data.object` is a union over every Stripe object type, so the id is
  // read only from the members that declare one.
  const object = event.data.object
  const objectId = 'id' in object && typeof object.id === 'string' ? object.id : null
  return { id: event.id, type: event.type, objectId }
}
