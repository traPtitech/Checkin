...
const ALL = 'all'
// This list is the single source for both the select items and the type of the
// filter, so the two cannot drift apart.
const PAYOUT_STATUSES = ['pending', 'onboarding_waiting', 'processing', 'paid', 'failed'] as const
type PayoutStatus = typeof PAYOUT_STATUSES[number]
const statusItems = [
  { value: ALL, label: 'すべて' },
  ...PAYOUT_STATUSES.map(s => ({ value: s, label: s })),
]

// The ref holds either a payout status or the `ALL` sentinel, so the request
...
const status = ref<PayoutStatus | typeof ALL>(ALL)

function onStatusChange(value: string) {
  status.value = PAYOUT_STATUSES.find(s => s === value) ?? ALL
}
...
async function onExecute(jomonRef: string) {
  ...
  try {
    ...
    executeResult.value = { ...executeResult.value, [jomonRef]: { ...result } }
    await loadList()
  }
  catch (e) {
...
function canMarkManuallyPaid(s: string): boolean {
  return MANUAL_PAYABLE.some(payable => payable === s)
}
...
async function onMarkManuallyPaid(jomonRef: string, note: string) {
  ...
  try {
    ...
    markPaidResult.value = { ...markPaidResult.value, [jomonRef]: { ...result } }
    await loadList()
  }
  catch (e) {
...
function writtenBack(value: PayoutView['jomonWrittenBackAt']): string {
