  ...
  <div class="max-w-md mx-auto space-y-6">
    ...
    >
      ...
      <div class="grid gap-3">
        <!--
          `onClick` goes through `v-bind` rather than `@click`: UCard declares no
          `click` emit, so the listener is a fallthrough attribute on its root.
          `strictTemplates` rejects undeclared attributes, and `fallthroughAttributes`
          cannot type them either because UCard's root is a `Primitive` whose tag
          comes from the `as` prop. A `v-bind` object is not checked against the
          declared props, so the listener reaches the root exactly as before.
        -->
        <UCard
          v-for="c in CATEGORIES"
          :key="c.value"
          v-bind="{ onClick: () => chooseCategory(c.value) }"
