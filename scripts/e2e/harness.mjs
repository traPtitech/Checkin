...
// Chromium binary. Unset by default, which leaves the resolution to
// playwright-core. playwright-core does not download browsers itself, so either
// install them (`npx playwright install chromium`, which playwright-core then
// finds in its cache) or point E2E_CHROMIUM at a Chromium/Chrome executable.
const EXE = process.env.E2E_CHROMIUM || undefined
// Screenshots land next to this file, so the path does not depend on where the
// repository is checked out. Git-ignored (see .gitignore).
