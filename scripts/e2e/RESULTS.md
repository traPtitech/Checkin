...
> 環境: dev サーバ（`CHECKIN_DEV_LOGIN=1`）/ Stripe test キー＋`stripe listen` / ローカルの Jomon v1 / 共有ハーネス `scripts/e2e/harness.mjs`。
...
> 注: 入-16（traQ 無しでも本人 mail_hash 一致で発行可）は入-04 の成功で事実観測済み。入-19①（Customer 新規作成）も入-04 でカバー。paid 拒否系（入-10/継-05/特-08/特-11）はこの検証で実カード決済により解決済み。
